...
export default function Login() {
  ...
  const registerPath = selectedRole === 'staff' ? '/register-staff' : '/register';
  const showRegisterLink = selectedRole !== 'admin';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authAPI.login(email, password, selectedRole || undefined);
      ...
      <div className="w-full max-w-sm">
        ...
        {showRegisterLink && (
          <div className="text-center mt-6">
            <Link to={registerPath} className="text-blue-600 hover:text-blue-700 text-sm font-medium">
              Switch to Registration
            </Link>
          </div>
        )}
